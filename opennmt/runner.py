...
class Runner(object):
  ...
  def _build_train_spec(self, checkpoint_path, source_scope=None, target_scope=None):
    train_hooks = [
        hooks.LogParametersCountHook()]

    if checkpoint_path is not None:
      train_hooks.append(hooks.LoadWeightsFromCheckpointHook(checkpoint_path, source_scope, target_scope))
    if self._hvd is not None:
  ...
  def train_and_evaluate(self, checkpoint_path=None, source_scope=None, target_scope=None):
    """Runs the training and evaluation loop.

    Args:
      checkpoint_path: The checkpoint path to load the model weights from it.
      source_scope: Checkpoint scope name to restore to model.
      target_scope: target scope name to restore to model.
    ...
    train_spec = self._build_train_spec(checkpoint_path, source_scope, target_scope)
  ...
  def train(self, checkpoint_path=None, source_scope=None, target_scope=None):
