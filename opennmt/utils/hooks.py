...
class LoadWeightsFromCheckpointHook(_SESSION_RUN_HOOK):
  ...
  def __init__(self, checkpoint_path, source_scope=None, target_scope=None):
  ...
  def begin(self):
    ...
    tf.logging.info("***load from {} ***".format(self.checkpoint_path))
    for variable in variables:
      name = variable.op.name
      if self.target_scope is not None:
        if name.startswith(self.target_scope):
          name = name.replace(self.target_scope, self.source_scope)
        else:
          continue
      if name in names:
