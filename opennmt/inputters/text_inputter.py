...
class MaskedWordEmbedder(WordEmbedder):
  """Masked word embedder."""
  ...
  def __init__(self,
               ...
               mask_unchanged_rate=0.1,
               mask_token=constants.MASK_TOKEN,
               accumulate_all_word_loss=False,
               infer_mask=True):
    """Initializes the parameters of the masked word embedder.
    ...
    Args:
      ...
      tokenizer: An optional :class:`opennmt.tokenizers.tokenizer.Tokenizer` to
        tokenize the input text. Defaults to a space tokenization.
      support_copy: bool. Whether support to copy. Defaults is False.
      mask_rate: mask word rate. float.
      mask_mask_rate: replace with <mask>.
      mask_random_rate: replace with random token.
      mask_unchanged_rate: don't replace.
      accumulate_all_word_loss: accumulate all word loss. bool.
      dtype: The embedding type.
    ...
    See Also:
      ...
      function for details about the pretrained embedding format and behavior.
    """
    super(MaskedWordEmbedder, self).__init__(
        vocabulary_file_key,
        embedding_size,
        embedding_file_key,
        embedding_file_with_header,
    ...
    self.mask_mask_rate = mask_mask_rate
    self.mask_random_rate = mask_random_rate
    self.mask_unchanged_rate = mask_unchanged_rate
    self.accumulate_all_word_loss = accumulate_all_word_loss
    self.infer_mask = infer_mask

  def initialize(self, metadata, asset_dir=None, asset_prefix=""):
    super(MaskedWordEmbedder, self).initialize(metadata, asset_dir, asset_prefix)
    self._masked_vocabulary = []
    for line in open(self.vocabulary_file):
      self._masked_vocabulary.append(line.strip())
  ...
  def mask_tokens_tf_fn(self, tokens):
    """mask tokens reference to bert use tf-api."""
    tokens_num = tf.shape(tokens)[0]
    rand_rate = tf.random.uniform([tokens_num])
    flag_0 = rand_rate < self.mask_rate * self.mask_mask_rate
    flag_1 = rand_rate < self.mask_rate * (self.mask_mask_rate + self.mask_random_rate)
    flag_2 = rand_rate < self.mask_rate
    output_mask_false = tf.zeros([tokens_num], dtype=tf.int32)
    output_mask_true = tf.ones([tokens_num], dtype=tf.int32)
    output_mask = tf.where(flag_2, x=output_mask_true, y=output_mask_false)
    masked_vocab_tf = \
        tf.convert_to_tensor(self._masked_vocabulary[constants.MASK_TOKEN_ID:])[:, None]
    output_tokens_random = _random_choice(masked_vocab_tf, tokens_num)[:, 0]
    output_tokens_mask = tf.tile([self.mask_token], [tokens_num])
    output_tokens = tf.where(flag_0,
                             x=output_tokens_mask,
                             y=tf.where(flag_1,
                                        x=output_tokens_random,
                                        y=tokens))
    return output_tokens, output_mask

  def make_features(self, element=None, features=None, training=None):
    """Converts words tokens to ids."""
    features = TextInputter.make_features(
        self,
        element=element, features=features, training=training)
    ...
    if "mask" not in features:
      mask_tokens, features["mask"] = self.mask_tokens_tf_fn(features["tokens"])
      if self.infer_mask or training:
        features["tokens"] = mask_tokens
      features["tokens"].set_shape([None])
      features["mask"].set_shape([None])
    ids = self.vocabulary.lookup(features["tokens"])
    if not self.is_target:
      features["ids"] = ids
    else:
      ...
      features["ids_out"] = tf.concat([ids, eos], axis=0)
      features["length"] += 1 # Increment length accordingly.
    features["copy_ids"] = features["ids"]
    return features
